...
import { Copy, Check, Share2, Users, Gift, ExternalLink } from "lucide-react";
...
export default function Referral() {
  ...
  const { data: codeData, isLoading: codeLoading } = trpc.referral.getMyReferralCode.useQuery();
  ...
  const shareWhatsApp = () => {
    const message = encodeURIComponent(
      `🚀 Acabei de descobrir o ResumAI - crie currículos profissionais com IA em minutos!\n\nCadastre-se usando meu link e ganhe créditos extras: ${referralLink}`
    );
  ...
  const shareTwitter = () => {
    const text = encodeURIComponent(
      `🚀 Crie currículos profissionais com IA no @ResumAI! Cadastre-se e ganhe créditos extras: ${referralLink}`
    );
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-muted-foreground">
            Compartilhe o ResumAI com seus amigos e ganhe <strong>+2 currículos extras</strong> para cada indicação bem-sucedida!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Recompensados</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-blue-600">{stats?.rewarded || 0}</div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Créditos Ganhos</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-purple-600">{stats?.totalCreditsEarned || 0}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Compartilhe este link com seus amigos. Quando eles se cadastrarem, você ganha +2 currículos extras!
            </CardDescription>
          </CardHeader>
            ...
            <ol className="space-y-4">
              ...
              <li className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Quando alguém criar uma conta usando seu link, a indicação é registrada
                  </p>
                </div>
              </li>
              <li className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Você recebe +2 currículos extras para cada amigo que se cadastrar!
                  </p>
                </div>
              </li>
            </ol>
                  ...
                  <div key={referral.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      <p className="font-medium">
                        {referral.referredUserName || referral.referredUserEmail || "Aguardando cadastro"}
                      </p>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <div className="text-right">
                      ...
                      {referral.rewardCredits > 0 && (
                        <p className="text-sm text-green-600 mt-1">+{referral.rewardCredits} créditos</p>
                      )}
                    </div>
                  </div>
                ))}
